...
/**
 * How much wider than its tightest crop a step may be shown so that turning the map doesn't push it
 * out of view.
 *
...
export class MapCropperImpl implements MapCropper{
    ...
    crop(navigationStep: NavigationStep, width: number, height: number): CentroidScale {
        ...
        const stepScale = this.rotationProofScale(
    ...
    /**
     * Widens a crop until the step stays inside it at whatever angle the map is drawn at.
     *
     * Compass mode turns the map under a view that stays the same shape, so a step only stays
     * inside it as long as the circle it spins in does - and that circle is as wide as the step's
     * diagonal. Every view is cropped this way, turning or not, so that switching the compass on
     * leaves the user looking at exactly as much map as before.
     */
    private rotationProofScale(
        scale: number,
        stepWidth: number,
        stepHeight: number,
        width: number,
        ...
        // Fit the whole step in when that is cheap, and settle for a step that reaches past the
        // edges when it isn't - see MAX_ROTATION_ZOOM_OUT.
        return Math.max(
