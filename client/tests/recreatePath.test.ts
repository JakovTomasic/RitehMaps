...
describe('testing recreatePath()', () => {
  ...
  test('should return correct recreated path', () => {
    ...
    var path = recreatePath(lastNodeId, parents, endNodesGraph);
    expect(path).toEqual(expectedPath);
  });

  test('should return error due to incorrect lastNodeId', () => {
    var lastNodeId = 10;
    var parents = new Map<number, number>([[2, 1], [3, 2], [4, 3], [10, 4]]);
    var errorMessage = "Node with id = 10 does not exist in the provided graph.";

    expect(() =>  recreatePath(lastNodeId, parents, endNodesGraph)).toThrowError(errorMessage);
  });
